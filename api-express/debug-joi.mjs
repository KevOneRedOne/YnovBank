...
const regResult = registrationSchema.validate({}, { abortEarly: false });
...
console.log('Erreurs:', withdrawalResult.error?.details.map(d => {
  if (d.type === 'any.custom' && d.context?.message) {
...
console.log('Erreurs:', depositResult.error?.details.map(d => {
  if (d.type === 'any.custom' && d.context?.message) {
    return d.context.message;
  }
  return d.message;
...
console.log('Erreurs:', transferResult.error?.details.map(d => {
  if (d.type === 'any.custom' && d.context?.message) {
    return d.context.message;
  }
  return d.message;
